...
class LianjiaSpider(scrapy.Spider):
    ...
    with open(
        "../2spider_link_bankuai/lianjia_shenzhen_processed.json", "r", encoding="utf-8"
    ) as f:
        url_list = [json.loads(line) for line in f]

    start_urls = ["https://sz.lianjia.com" + url_list[0]["link_district"]]
    ...
    def parse(self, response, **kwargs):
        ...
        if self.url_list: # if url_list is not empty
            url = "https://sz.lianjia.com" + self.url_list[0]["link_district"] + "pg" + str(self.page_index)
